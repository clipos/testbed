...
readonly IMAGE_TYPE="${3:?IMAGE_TYPE is needed}"
readonly IMAGE_SIZE="${4:?IMAGE_SIZE is needed}"

readonly IMAGE="$(basename ${IMAGE_FILE})"

echo "${IMAGE}: Creating disk image as ${IMAGE_TYPE} ${IMAGE_SIZE}..."
...
echo "${IMAGE}: Getting blockdev-getsz..."
...
echo "${IMAGE}: Getting blockdev-getsz: OK"
...
echo "${IMAGE}: Creating disk and partitions..."
guestfish --rw <<_EOF_
add-drive ${IMAGE_FILE} label:main
...
_EOF_
echo "${IMAGE}: Creating disk and partitions: OK"

echo "${IMAGE}: Creating vfat on EFI & LVM..."
guestfish --rw <<_EOF_
add-drive ${IMAGE_FILE} label:main
...
_EOF_
echo "${IMAGE}: Creating vfat on EFI & LVM: OK"

chmod a+rw "${IMAGE_FILE}"

echo "${IMAGE}: Creating disk image as ${IMAGE_TYPE} ${IMAGE_SIZE}: OK"
